...
class Migration(SchemaMigration):
    ...
    def forwards(self, orm):
        # Removing unique constraint on 'Match', fields ['tournament', 'slug', 'collection']
        db.delete_unique('matches_match', ['tournament_id', 'slug', 'collection_id'])

        # Adding field 'Tournament.slug'
        db.add_column('matches_tournament', 'slug',
                      self.gf('django.db.models.fields.CharField')(default='slug', max_length=63),
                      keep_default=False)

        # Deleting field 'Match.tournament'
        db.delete_column('matches_match', 'tournament_id')

        # Adding unique constraint on 'Match', fields ['slug', 'collection']
        db.create_unique('matches_match', ['slug', 'collection_id'])

        # Adding unique constraint on 'Game', fields ['game_number', 'match']
        db.create_unique('matches_game', ['game_number', 'match_id'])

        # Adding field 'Collection.slug'
        db.add_column('matches_collection', 'slug',
                      self.gf('django.db.models.fields.CharField')(default='slug', max_length=63),
                      keep_default=False)

    def backwards(self, orm):
        # Removing unique constraint on 'Game', fields ['game_number', 'match']
        db.delete_unique('matches_game', ['game_number', 'match_id'])

        # Removing unique constraint on 'Match', fields ['slug', 'collection']
        db.delete_unique('matches_match', ['slug', 'collection_id'])

        # Deleting field 'Tournament.slug'
        db.delete_column('matches_tournament', 'slug')


        # User chose to not deal with backwards NULL issues for 'Match.tournament'
        raise RuntimeError("Cannot reverse this migration. 'Match.tournament' and its values cannot be restored.")
        # Adding unique constraint on 'Match', fields ['tournament', 'slug', 'collection']
        db.create_unique('matches_match', ['tournament_id', 'slug', 'collection_id'])

        # Deleting field 'Collection.slug'
        db.delete_column('matches_collection', 'slug')
